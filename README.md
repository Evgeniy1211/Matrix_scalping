...
### 🗂️ Единая структура данных (DATABASE UNIFICATION ✅)

### backend/data/ - ЕДИНЫЙ ИСТОЧНИК ПРАВДЫ
...
- ✅ **Единый источник данных**: все данные в `backend/data/`
- ✅ **Никакого дублирования**: `backend/knowledge-base/` содержит только схемы и документацию
- ✅ **Фронтенд использует только API**: данные поступают через REST эндпоинты
- ✅ **Типизация через `@shared/schema`**: общие типы для фронтенда и бэкенда
- ✅ **Валидация данных**: автоматическая проверка консистентности

### API эндпоинты:

- `GET /api/modules` → список всех модулей
- `GET /api/modules/:id` → конкретный модуль по названию
- `GET /api/technologies` → база технологий
- `GET /api/trading-machines` → кейсы торговых машин
- `GET /api/evolution-data` → матрица эволюции (оригинальная)
- `GET /api/evolution-data/integrated` → интегрированная матрица
- `GET /api/evolution-data/dynamic` → динамическая матрица
- `GET /api/tree-data` → дерево технологий
