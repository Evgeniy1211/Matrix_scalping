...
export function TradingMachineComparator() {
  ...
  const [selectedCase, setSelectedCase] = useState<TradingMachine | null>(null);

  // Set first trading machine as default when data loads - PROPER useEffect
  useEffect(() => {
    if (tradingMachines?.length && !selectedCase) {
      setSelectedCase(tradingMachines[0]);
    }
  }, [tradingMachines, selectedCase]);
  ...
  // No data available
  if (!tradingMachines || tradingMachines.length === 0) {
    return (
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        <Card>
          <CardHeader>
          ...
          <CardContent>
            <div className="text-center py-8 text-muted-foreground">Нет доступных кейсов</div>
          </CardContent>
        </Card>
      </div>
    );
